...
const STATUS_TONE: Record<string, 'info' | 'success' | 'warning' | 'danger'> = {
  OPEN: 'warning', RESOLVED: 'success', CLOSED: 'info', IN_PROGRESS: 'info',
};
...
export default function SupportTickets() {
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      Alert.alert('Could not create ticket', 'Please check your connection and try again.');
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Pressable onPress={() => router.push('/support/faq')} style={styles.faqBtn}>
      ...
      <View style={styles.contactRow}>
        {[
          { icon: 'chatbubble-outline' as const, label: 'Chat', color: colors.primary },
          { icon: 'call-outline' as const, label: 'Call', color: colors.success },
          { icon: 'mail-outline' as const, label: 'Email', color: colors.warning },
        ].map((c) => (
      ...
      <View style={styles.ticketsHeader}>
        ...
        <Pressable style={styles.newTicketBtn} onPress={() => setShowCreate(true)}>
          ...
          renderItem={({ item }) => (
            <View style={styles.ticketCard}>
              <View style={styles.ticketTop}>
                ...
                <StatusPill label={item.status} tone={STATUS_TONE[item.status] ?? 'info'} />
              </View>
            </View>
          )}
          ListEmptyComponent={
            <EmptyState icon="ticket-outline" title="No tickets yet" subtitle="Raise a ticket if you need help" />
          }
        />
      )}
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={{ padding: spacing.xl }}>
              ...
              />
              <Button title="Submit Ticket" onPress={createTicket} loading={creating} style={{ marginTop: spacing.xl }} />
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  faqBtn: { paddingHorizontal: spacing.md, paddingVertical: spacing.xs, backgroundColor: colors.primaryLight, borderRadius: radius.pill },
  faqBtnText: { fontSize: fontSize.sm, fontWeight: fontWeight.bold, color: colors.primary },
  ...
  contactLabel: { fontSize: fontSize.xs, fontWeight: fontWeight.semibold, color: colors.textPrimary },
  ticketsHeader: {
  ...
  newTicketText: { fontSize: fontSize.sm, fontWeight: fontWeight.bold, color: colors.primary },
  list: { paddingHorizontal: spacing.xl, gap: spacing.md, paddingBottom: spacing.xxxl },
  ticketCard: { backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.lg, ...shadow.subtle },
  ticketTop: { flexDirection: 'row', alignItems: 'center', gap: spacing.md },
  ticketIcon: { width: 44, height: 44, borderRadius: radius.md, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center' },
  ticketSubject: { fontSize: fontSize.sm, fontWeight: fontWeight.bold, color: colors.textPrimary },
  ...
  msgLabel: { fontSize: fontSize.sm, fontWeight: fontWeight.semibold, color: colors.textPrimary, marginBottom: spacing.sm },
  msgInput: {
